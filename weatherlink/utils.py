...
def calculate_dew_point(temperature, relative_humidity):
    ...
    #global errors_inside_programm
    if relative_humidity < 0:
        text = f"function calculate_dew_point: relative_humidity = {relative_humidity}, temperature = {temperature}"
        print(text)
        #errors_inside_programm += text
        #device.write_to_bot(f"{davis.device_name} Convertor: {error}")
        return temperature.quantize(ONE_TENTH)
    
    Tc = convert_fahrenheit_to_celsius(temperature)
    RH = _as_decimal(relative_humidity)
    #print(Tc, RH, DP_B, DP_D, DP_C)
    xx = ((DP_B - (Tc / DP_D)) * (Tc / (DP_C + Tc)))
    #print("exp", xx.exp(), math.exp(xx))
    #print("explog", (xx.exp() * RH / 100).ln())
...
def calculate_all_record_values(record):
    ...
    if humidity_outside:
        a = []
        b = []
        #print("33", (temperature_outside, humidity_outside))
        if temperature_outside:
        ...
        #print("333", temperature_outside_low, humidity_outside)
        if temperature_outside_low:
        ...
        #print("333", (temperature_outside_high, humidity_outside))
        if temperature_outside_high:
